...
function popStack(stack) {
  ...
  // Like in `Container#each()`, the parent’s cursor stays on the current
  // child while its subtree is processed and moves to the next child only
  // when the subtree is done, so nodes inserted after the current child
  // are visited in the same pass
  if (parent && parent.iterator !== 0) {
    ...
    while (stack.length > 0) {
      ...
      if (visit.eventIndex < visit.events.length) {
        ...
        if (event === CHILDREN) {
          ...
          if (visitors) {
            if (this.visitSync(visitors, visitNode.toProxy())) popStack(stack)
          }
        }
        continue
      }
